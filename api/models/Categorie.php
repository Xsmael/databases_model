...
class   Categorie {
	...
	public function read($obj) {
		$sth = $this->dbh->prepare("SELECT * FROM categorie WHERE ID=?");
	...
	public function create($obj) {		
		$sth = $this->dbh->prepare("INSERT INTO categorie (ID,nom) VALUES (?,?);");
	...
	public function update($obj) {		
		$sth = $this->dbh->prepare("UPDATE categorie SET ID = IF(? = NULL, ID , ?), nom = IF(? = NULL, nom , ?) WHERE ID=?");
		$sth->execute(array($obj->ID,$obj->ID,$obj->nom,$obj->nom,$obj->ID));
		return json_encode(1);
	}
	
	public function delete($obj) {				
		$sth = $this->dbh->prepare("DELETE FROM categorie WHERE ID=?");
		$sth->execute(array($obj->ID));
		return json_encode(1);
	}
}
