...
class   Diplome {
	...
	public function readAll() {				
		$sth = $this->dbh->prepare("SELECT * FROM diplome");
		$sth->execute();
		return json_encode($sth->fetchAll(PDO::FETCH_ASSOC));
	}
	
	public function read($obj) {
		$sth = $this->dbh->prepare("SELECT * FROM diplome WHERE ID=?");
	...
	public function create($obj) {		
		$sth = $this->dbh->prepare("INSERT INTO diplome (ID,nom) VALUES (?,?);");
	...
	public function update($obj) {		
		$sth = $this->dbh->prepare("UPDATE diplome SET ID = IF(? = NULL, ID , ?), nom = IF(? = NULL, nom , ?) WHERE ID=?");
		$sth->execute(array($obj->ID,$obj->ID,$obj->nom,$obj->nom,$obj->ID));
		return json_encode(1);
	}
	
	public function delete($obj) {				
		$sth = $this->dbh->prepare("DELETE FROM diplome WHERE ID=?");
		$sth->execute(array($obj->ID));
		return json_encode(1);
	}
}
